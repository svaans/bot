...
def triple_top(df: pd.DataFrame) -> dict:
    ...
    # Buscar tres picos similares
    p1, p2, p3 = highs[5], highs[15], highs[25]
    tolerancia = 0.015 * df["high"].mean()

    son_similares = abs(p1 - p2) < tolerancia and abs(p2 - p3) < tolerancia and abs(p1 - p3) < tolerancia

    # Confirmar que el soporte está plano (últimos mínimos similares)
