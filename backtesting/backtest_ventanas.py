...
def cargar_historicos(symbols: Iterable[str], ruta_datos: str = "datos") -> Dict[str, pd.DataFrame]:
    datos = {}
    for s in symbols:
        archivo = f"{ruta_datos}/{s.replace('/', '_').lower()}_1m.parquet"
        df = pd.read_parquet(archivo).dropna().sort_values("timestamp").reset_index(drop=True)
        datos[s] = df
    return datos
...
def backtest_en_ventanas(symbol: str, n_ventanas: int = 3, n_trials: int = 20) -> List[Tuple[dict, float]]:
