...
def cargar_historicos(symbols: Iterable[str], ruta: str = RUTA_DATOS) -> Dict[str, pd.DataFrame]:
...
async def procesar_df(bot: TraderSimulado, symbol: str, df: pd.DataFrame) -> None:
...
async def ejecutar_backtest(symbol: str, dfs: List[pd.DataFrame], config: dict, pesos: dict) -> TraderSimulado:
    """Ejecuta el backtest para un símbolo con su configuración y pesos."""
    bot = TraderSimulado([symbol], configuraciones={symbol: config}, pesos_personalizados={symbol: pesos}, modo_optimizacion=True)
    for df in dfs:
...
def evaluar(bot: TraderSimulado, symbol: str) -> dict:
    ...
    sharpe = resultados.mean() / resultados.std() * np.sqrt(len(resultados)) if resultados.std() > 0 else 0.0
    ratio_br = (ganancias.mean() / -perdidas.mean()) if len(perdidas) and len(ganancias) else 0.0
    return {
        "capital": capital_final,
        "ganancia": capital_final - capital_inicial,
...
def optimizar_symbol(symbol: str, df: pd.DataFrame, estrategias: List[str]) -> tuple[dict, dict, dict]:
    ...
    def objective(trial: optuna.Trial) -> float:
        config_symbol = {
            "factor_umbral": trial.suggest_float("factor_umbral", 0.7, 2.0),
            "ajuste_volatilidad": trial.suggest_float("ajuste_volatilidad", 0.0, 1.0),
            "riesgo_maximo_diario": trial.suggest_float("riesgo_maximo_diario", 0.01, 0.10),
            "peso_minimo_total": trial.suggest_float("peso_minimo_total", 0.5, 3.0),
            "diversidad_minima": trial.suggest_int("diversidad_minima", 1, 5),
            "cooldown_tras_perdida": trial.suggest_int("cooldown_tras_perdida", 1, 10),
            "sl_ratio": trial.suggest_float("sl_ratio", 0.002, 0.03),
            "tp_ratio": trial.suggest_float("tp_ratio", 0.002, 0.04),
            "ratio_minimo_beneficio": trial.suggest_float("ratio_minimo_beneficio", 1.0, 2.5),
            "multiplicador_estrategias_recurrentes": trial.suggest_float("multiplicador_estrategias_recurrentes", 0.0, 2.0),
        ...
        pesos = {e: trial.suggest_float(f"peso_{e}", 0.0, 2.0) for e in estrategias}
        bot = asyncio.run(ejecutar_backtest(symbol, bloques, config_symbol, pesos))
    ...
    return study.best_params, study.best_trial.user_attrs["metricas"], {
        k.replace("peso_", ""): v for k, v in study.best_params.items() if k.startswith("peso_")
    }
...
if __name__ == "__main__":
    ...
    for symbol, df in datos.items():
        ...
        configuraciones[symbol] = {
            k: v for k, v in config.items() if not k.startswith("peso_")
        }
