...
class BacktestRunner:
    ...
    def __init__(self, config: BacktestConfig) ->None:
        ...
        self._load_data()
        trader_cfg = Config(api_key='', api_secret='', modo_real=False,
            intervalo_velas=self.config.intervalo, symbols=self.config.
            symbols, umbral_riesgo_diario=0.05, min_order_eur=10.0,
    ...
    def _load_data(self) ->None:
        for sym in self.config.symbols:
            path = (
                f"{self.config.data_dir}/{sym.replace('/', '_').lower()}_{self.config.intervalo}.parquet"
                )
            if not os.path.isfile(path):
                raise FileNotFoundError(
                    f'Datos no encontrados para {sym}: {path}')
            df = pd.read_parquet(path).dropna().sort_values('timestamp'
            ...
            for i in range(max_len):
                ...
                for sym, df in self.data.items():
                    if i >= len(df):
                        continue
                    row = df.iloc[i]
                    vela = {'symbol': sym, 'timestamp': row['timestamp'],
                        'open': row['open'], 'high': row['high'], 'low':
                        row['low'], 'close': row['close'], 'volume': row[
                        'volume']}
