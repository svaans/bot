...
def calcular_cruce_ema_stochrsi(df: pd.DataFrame, ema_fast=9, ema_slow=21, rsi_period=14, stoch_period=14, d_period=3):
    if "close" not in df or len(df) < max(ema_slow, rsi_period + stoch_period + d_period):
        return False
    ...
    cruce_ema = ema_rapida.iloc[-2] < ema_lenta.iloc[-2] and ema_rapida.iloc[-1] > ema_lenta.iloc[-1]

    # RSI
    delta = df["close"].diff()
    ganancia = delta.where(delta > 0, 0.0)
    perdida = -delta.where(delta < 0, 0.0)
    media_ganancia = ganancia.rolling(window=rsi_period).mean()
    media_perdida = perdida.rolling(window=rsi_period).mean()
    ...
    cruce_stochrsi = stoch_k.iloc[-2] < stoch_d.iloc[-2] and stoch_k.iloc[-1] > stoch_d.iloc[-1]
