...
Como Revolut Robo-Advisor, cuando la cartera se desvía del target
(ej. BTC creció y ahora pesa 40% en vez del 30% objetivo), el rebalancer
detecta el drift y sugiere vender BTC y comprar el símbolo sub-pesado.

Algoritmo:
1. Calcular valor actual de cada posición aberta
2. Calcular peso actual vs peso objetivo
3. Si drift > umbral → generar acciones de rebalanceo
4. Acciones: {"symbol": ..., "accion": "reducir"|"ampliar", "drift_pct": ...}

No ejecuta órdenes directamente — genera recomendaciones que el trader
puede actuar. Thread-safe. Sin dependencias externas.

Persiste pesos objetivo en estado/portfolio_target.json
"""
...
_TARGET_PATH = Path("estado/portfolio_target.json")

_DEFAULT_SYMBOLS = ["BTC/USDT", "ETH/USDT", "SOL/USDT", "XRP/USDT", "AVAX/USDT"]
_EQUAL_WEIGHT = 1.0 / len(_DEFAULT_SYMBOLS)  # 0.20 cada uno
...
def cargar_pesos_objetivo(symbols: list[str] | None = None) -> dict[str, float]:
    """Carga pesos objetivo desde archivo o retorna igual weight."""
    try:
...
def calcular_drift(
    ...
    Args:
        valores_actuales: {symbol: valor_en_quote (ej. USDT)}
        pesos_objetivo:   {symbol: peso 0-1}

    Returns:
        {symbol: drift} donde drift > 0 = sobrepondero, < 0 = subpondero
    """
...
def generar_acciones_rebalanceo(
    ...
    Returns lista de dicts:
        {"symbol": str, "accion": "reducir"|"ampliar",
         "drift_pct": float, "usdt_delta": float}
    """
    ...
    # Ordenar por urgencia (mayor drift absoluto primero)
...
def resumen_cartera(
    ...
    for sym in sorted(pesos_objetivo):
        ...
        emoji = "✅" if abs(d) <= 5 else ("🔴" if d > 0 else "🔵")
        lines.append(
            f"  {emoji} {sym:12} | actual={peso_actual:5.1f}% "
