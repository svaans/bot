...
def verificar_trailing_stop(info: dict, precio_actual: float, df: (pd.
    ...
    trailing_start_ratio = config.get('trailing_start_ratio', 1.015
        ) if config else 1.015
    atr_mult = config.get('atr_multiplicador', 1.0) if config else 1.0
    ...
    if max_price >= trailing_trigger:
        if usar_atr:
            ...
            distancia_ratio = config.get('trailing_distance_ratio', 0.02
                ) if config else 0.02
            trailing_stop = max_price * (1 - distancia_ratio)
        if config.get('uso_trailing_technico', False
            ) and df is not None and len(df) >= 5:
