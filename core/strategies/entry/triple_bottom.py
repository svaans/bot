...
def triple_bottom(df: pd.DataFrame) -> dict:
    ...
    # Suponemos los mínimos en 3 puntos clave
    p1, p2, p3 = lows[5], lows[15], lows[25]
    tolerancia = 0.015 * df["low"].mean()

    minimos_similares = (
        abs(p1 - p2) < tolerancia and
        abs(p2 - p3) < tolerancia and
        abs(p1 - p3) < tolerancia
    )

    # Verificamos que la resistencia esté casi estable en los últimos 5 máximos
