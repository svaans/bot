...
def hay_contradicciones(estrategias_activas: Dict[str, bool]) -> bool:
    ...
    tiene_alcista = any(
        "alcista" in nombre and activo for nombre, activo in estrategias_activas.items()
    )
    tiene_bajista = any(
        "bajista" in nombre and activo for nombre, activo in estrategias_activas.items()
    )
