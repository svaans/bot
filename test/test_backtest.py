...
def generar_velas(n=40, symbol='BTC/EUR'):
    ...
    for i, t in enumerate(ts):
        vela = {'symbol': symbol, 'timestamp': int(t.timestamp() * 1000),
            'open': precio, 'high': precio + 1, 'low': precio - 1, 'close':
            precio + 1, 'volume': 10}
        velas.append(vela)
        precio += 1
    return velas
...
def crear_trader(monkeypatch):
    monkeypatch.setattr('core.trader_modular.crear_cliente', lambda cfg:
        DummyClient())
    monkeypatch.setattr('binance_api.cliente.crear_cliente', lambda cfg=
        None: DummyClient())
    ...
    monkeypatch.setattr('core.ordenes_reales.obtener_todas_las_ordenes', lambda
        : {})
    monkeypatch.setattr('core.ordenes_reales.sincronizar_ordenes_binance', 
        lambda symbols: {})
    monkeypatch.setattr('core.trader_modular.calcular_fraccion_kelly', lambda :
        1.0)
    monkeypatch.setattr('pandas.read_parquet', lambda *a, **k: pd.DataFrame
        ({'close': [1, 2, 3, 4]}))
    monkeypatch.setattr('core.trader_modular.fetch_balance_async',
        fake_fetch_balance_async)
    cfg = Config(api_key='', api_secret='', modo_real=False,
        intervalo_velas='1m', symbols=['BTC/EUR'], umbral_riesgo_diario=0.1,
        min_order_eur=10, persistencia_minima=1, usar_score_tecnico=False)
