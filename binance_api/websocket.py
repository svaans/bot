...
# Lista de intervalos válidos de Binance, puedes ampliar
    ...
    while True:
        try:
            ws = await websockets.connect(
                ...
                ping_interval=20,
                ping_timeout=20,
                max_size=2**20  # 1 MB por mensaje como límite de seguridad
            )
            ...
            try:
                async for msg in ws:
                    try:
                        ...
                        # comprobar que el evento es un kline
                        if data.get("e") != "kline":
                            log.debug(f"⚠️ Evento no esperado en {symbol}: {data.get('e')}")
                            continue

                        vela = data["k"]
                        if vela["x"]:  # vela cerrada
                            log.info(
            ...
            intentos += 1
            total_reintentos += 1
            espera = min(60, 5 * intentos)  # progresivo hasta 1 min
            log.warning(f"❌ Error en WebSocket de {symbol}: {e}")
            traceback.print_exc()
            log.info(f"🔁 Reintentando conexión en {espera} segundos... (total reintentos: {total_reintentos})")
...
async def _watchdog(ws, symbol, tiempo_maximo=300):
    ...
    try:
        ...
        log.warning(f"⚠️ No se recibieron velas en {tiempo_maximo}s para {symbol}, forzando reconexión.")
        await ws.close()
    except Exception:
        pass
