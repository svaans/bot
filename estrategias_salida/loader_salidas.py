# estrategias_salida/loader_salidas.py
import os
import importlib.util
import inspect

CARPETA_SALIDAS = "estrategias_salida"
        ...
        if archivo.startswith("salida_") and archivo.endswith(".py"):
            ...
            try:
                ...
                for attr in dir(modulo):
                    if attr.startswith("salida_") or (attr.startswith("verificar_") and attr not in ["verificar_trailing_stop", "verificar_reversion_tendencia", "evaluar_evitar_stoploss"]):
                        funcion = getattr(modulo, attr)
                        if callable(funcion) and es_estrategia_salida_valida(funcion):
                            funciones.append(funcion)
                        else:
                            print(f"⚠️ {nombre_modulo}.{attr} ignorada: no es función de salida válida.")
