...
class BotStopView(APIView):
    ...
    def post(self, request):
        ...
        BotState.objects.update_or_create(usuario=request.user, defaults={
            'activo': False})
...
class ConfigUploadView(APIView):
    ...
    def post(self, request):
        try:
            ...
            return Response({'detail': 'JSON inválido'}, status=status.
                HTTP_400_BAD_REQUEST)
        if not isinstance(configs, list):
            return Response({'detail': 'Formato incorrecto'}, status=status
                .HTTP_400_BAD_REQUEST)
        for item in configs:
            serializer = BotConfigSerializer(data=item)
            if serializer.is_valid():
                BotConfig.objects.update_or_create(usuario=request.user,
                    symbol=item.get('symbol'), defaults=serializer.
                    validated_data)
...
class MetricsView(APIView):
    ...
    def get(self, request):
        ...
        total = len(ordenes)
        if not total:
            return Response({'ganancia_acumulada': 0, 'winrate': 0,
                'operaciones': 0, 'saldo_actual': 0})
        ganancia = 0.0
        exitosas = 0
        for o in ordenes:
        ...
        return Response({'ganancia_acumulada': round(ganancia, 4),
            'winrate': round(winrate, 2), 'operaciones': total,
